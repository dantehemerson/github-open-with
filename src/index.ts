...
import '../css/extension_styles.css'
...
const init = async () => {
  ...
  // Tell every page that looks like a Extension xd installation that the browser extension is installed!
  const { extensionName } = configProvider.getConfig()
  const checkElement = document.getElementById(`ExtensionCheck_${extensionName}BrowserExtension`)
  if (checkElement) {
